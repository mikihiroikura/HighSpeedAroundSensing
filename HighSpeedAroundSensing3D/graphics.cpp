...
void update_pts(vector<vector<double>> pts) {
    //点群の位置更新
    if (pts.size()!=0)
    {
        //vector<vector<double>> last_pts = logs->LSM_pts[0];
        ...
        {
            //vertices.emplace(last_pts[i]*0.001);
            verts[savecnt][i][0] = pts[i][0] * 0.001;
            verts[savecnt][i][1] = pts[i][1] * 0.001;
            verts[savecnt][i][2] = pts[i][2] * 0.001;
            dist = hypot(verts[savecnt][i][0], verts[savecnt][i][1]);
            if (dist > safe_area) {
                colos[savecnt][i][1] = 0.0;
                colos[savecnt][i][2] = 0.0;
...
//OpenGLの描画
void drawGL_one(vector<vector<double>> pts) {
    //時間計測開始
    QueryPerformanceCounter(&glstart);
    ...
    if (pts.size() != 0)
    {
        ...
        for (size_t i = 0; i < 300; i++)
        {
            ...
            {
                verts[savecnt][i][0] = 0.0;
                verts[savecnt][i][1] = 0.0;
                verts[savecnt][i][2] = 0.0;
            }
            dist = hypot(verts[savecnt][i][0], verts[savecnt][i][1]);
            if (dist > safe_area) {
                colos[savecnt][i][1] = 0.0;
